            ...
            if (tabMode)
            {
                using (var craftContentChild = ImRaii.Child("CraftContent", size * ImGui.GetIO().FontGlobalScale))
                {
                    if (craftContentChild.Success)
                    {
                        ...
                        {
                            using var tabItem = ImRaii.TabItem(T("No Items"));
                            if (!tabItem.Success) return messages;
                            ImGui.TextWrapped(
                                T("No items have been added to the list. Add items via the search menu button at the top right of the screen or by right clicking on an item anywhere within the plugin."));
                        }
                        else
                        {
