...
public class ScopePickerColumnSetting : ColumnSetting<List<InventorySearchScope>?>
{
    ...
    public override bool Draw(ColumnConfiguration configuration, string? helpText)
    {
        ...
        if (helpText != null)
        {
            ...
            ImGuiUtil.HoverTooltip(T("Please make sure you include at least one inventory that contains crystals otherwise the craft calculator will not work."));
        }

        var currentValue = CurrentValue(configuration);
        using var disabled = ImRaii.Disabled(currentValue == null);
        if (ImGui.Button(T("Test Scopes")))
        {
        ...
        {
            ImGui.Separator();
            ImGui.Text(T("The following inventories will be searched in: "));
