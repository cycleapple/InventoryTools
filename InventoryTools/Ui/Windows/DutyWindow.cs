...
namespace InventoryTools.Ui
{
    class DutyWindow : UintWindow
    {
        ...
        public override void Draw()
        {
            if (ContentFinderCondition == null)
            {
                ImGui.TextUnformatted(T("Dungeon with the ID ") + _contentFinderConditionId + " could not be found.");
            }
            else
            {
                ...
                ImGui.TextUnformatted(T("Level Required: ") + ContentFinderCondition.Base.ClassJobLevelRequired);
                ImGui.TextUnformatted(T("Item Level Required: ") + ContentFinderCondition.Base.ItemLevelRequired);
                ;
                ...
                foreach (var dungeonBoss in DungeonBosses)
                {
                            ...
                            ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                    {
                        if (DungeonBossChests.ContainsKey(dungeonBoss.FightNo))
                        {
                            ...
                            {
                                if (ImGui.CollapsingHeader(T("Coffer ") + (chest.Key + 1),
                                        ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                                {
                        ...
                        if (DungeonBossDrops.ContainsKey(dungeonBoss.FightNo))
                        {
                            ...
                            if (ImGui.CollapsingHeader(T("Drops"), ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                            {
                                ImGuiStylePtr style = ImGui.GetStyle();
                                float windowVisibleX2 =
                                    ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
                ...
                if (ImGui.CollapsingHeader(T("Other Chests (") + DungeonChestItems.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                {
                    ImGuiStylePtr style = ImGui.GetStyle();
                ...
                if (ImGui.CollapsingHeader(T("Rewards (") + DungeonRewards.Count + ")", ImGuiTreeNodeFlags.DefaultOpen | ImGuiTreeNodeFlags.CollapsingHeader))
                {
                    ImGuiStylePtr style = ImGui.GetStyle();
                    float windowVisibleX2 = ImGui.GetWindowPos().X + ImGui.GetWindowContentRegionMax().X;
                ...
                {
                    ImGui.TextUnformatted(T("Duty ID: ") + _contentFinderConditionId);
